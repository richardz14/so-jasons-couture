...
if(!function_exists('qode_general_options_map')) {
    ...
    function qode_general_options_map(){
		...
		$disable_google_fonts = new QodeField("yesno","disable_google_fonts","no","Disable Google Fonts","Please note that in order to be presented with the full set of available options that changing this option entails, you should save your changes after triggering the option, and refresh the page afterwards.", array(),
		...
		$google_fonts = new QodeField("font", "google_fonts", "-1", "Font Family", "Choose a default font for your site");
        $panel1->addChild("google_fonts", $google_fonts);

		$disable_google_fonts_container = new QodeContainer("disable_google_fonts_container","disable_google_fonts","yes");
		...
		qode_add_repeater_field(
			array(
				'name'        => 'qode_custom_fonts',
				'label'       => esc_html__('Custom Fonts', 'qode'),
				...
				'fields' => array(
					array(
						...
						'type'        => 'file',
						'label'       => esc_html__('Custom Font TTF', 'qode'),
						'col_width'   => '3',
					),
					array(
						'name'        => 'qode_custom_font_otf',
						'type'        => 'file',
						'label'       => esc_html__('Custom Font OTF', 'qode'),
						'col_width'   => '3',
					),
					array(
						'name'        => 'qode_custom_font_woff',
						'type'        => 'file',
						'label'       => esc_html__('Custom Font WOFF', 'qode'),
						'col_width'   => '3',
					),
					array(
						'name'        	=> 'qode_custom_font_woff2',
						'type'        	=> 'file',
						'label'      	=> esc_html__('Custom Font TTF WOFF2', 'qode'),
						'col_width'   => '3',
					),
					array(
						'name'        => 'qode_custom_font_name',
						'type'        => 'text',
						'label'       => esc_html__('Font Name', 'qode'),
						'col_width'   => '3',
					)
        ...
        $lines_container = qode_add_admin_container(
            array(
                'parent'          => $panel1,
        ...
        $page_transitions_notice = new QodeNotice("Ajax Page Transition", 'Choose a a type of transition between loading pages. In order for animation to work properly, you must choose "Post name" in permalinks settings', "AJAX Page transitions are disabled due to VC Grid Elements", "enable_grid_elements", "no");
		...
		$page_loading_effect = new QodeField("yesno","page_loading_effect","no","Page Loading Effect","Choose a loading effect for your pages. Please note that this is not the same as Ajax page transitions. The page is loaded normally, without Ajax, but it will have a loading effect.");
		...
		$loading_background_color = new QodeField("color", "loading_background_color", "", "Background Color", 'Choose Background Color (Note: This option is only applied when Page Loading Effect is enabled)');
		...
		$google_maps_api_key = new QodeField("text", "google_maps_api_key", "", "Google Maps Api Key", "Insert your Google Maps API key here. For instructions on how to create a Google Maps API key, please refer to our to our documentation.");
