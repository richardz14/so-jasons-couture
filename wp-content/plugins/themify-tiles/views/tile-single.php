			...
			<div class="tile-inner">
				...
				if( $args['type_' . $side] == 'text' ) : ?>
					...
					<div class="tile-content">
						<?php echo apply_filters( 'themify_tiles_module_content', $args['text_' . $side] ); ?>
					</div><!-- .tile-content -->
				...
				elseif( $args['type_' . $side] == 'button' ) : ?>
					...
					<?php if( $args["button_link_{$side}"] != '' ) printf( '<a href="%s" class="%s" %s>', $url, $class, $target ); ?>
						...
						<?php if( $args["icon_type_{$side}"] == 'icon' && $args["icon_{$side}"] != '' ) : ?>
							<?php $icon_class = substr( $args["icon_{$side}"], 0, 3 ) === 'fa-' ? themify_get_fa_icon_classname( $args["icon_{$side}"] ) : $args["icon_{$side}"]; ?>
							<span class="tile-icon <?php echo $icon_class; ?>" style="<?php echo $icon_style; ?>"></span>
				...
				elseif( $args['type_' . $side] == 'map' ) : ?>
					...
					$num = rand( 0,10000 );
					$map_args = array(
						$args["address_map_{$side}"],
						$num,
						$args["zoom_map_{$side}"],
						$args["type_map_{$side}"],
						$args["scrollwheel_map_{$side}"] != 'enable' ? 'false' : 'true',
						$args["draggable_map_{$side}"] != 'enable' ? 'false' : 'true',
						
					);
					?>
					<div id="themify_map_canvas_<?php echo $num; ?>" style="" class="map-container" data-map="<?php echo esc_attr( json_encode( $map_args ) ); ?>">&nbsp;</div>
