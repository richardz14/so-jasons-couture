...
function themify_builder_get_control_binding_data($field) {
    ...
    if (isset($field['type'])) {
        ...
        if (in_array($default_type, array('text','range','radio', 'textarea', 'select_menu', 'select', 'selectbasic', 'image', 'icon','widgetized_select'), true)) {
...
function themify_builder_module_create_range_field($field,$is_style=false,$isChild = false){
    ...
    ?>
    <div<?php if( isset( $field['wrapper_class'] )):?> class="<?php echo $field['wrapper_class']?>"<?php endif;?>>
        <span class="tb_range_input">
                <input autocomplete="off"<?php if(!$has_units):?> data-max="500" data-min="0" data-increment="1"<?php endif;?> name="<?php echo $field['id']; ?>" <?php echo $input_id?> class="<?php if(isset($field['class'] )):?><?php echo $field['class']; ?> <?php endif;?>tb_lb_option tb_range<?php if($isChild):?> tb_lb_option_child<?php endif;?>" value="<?php if ( isset($field['value']) && $field['value'] !== false ) echo esc_attr($field['value']); ?>" type="text"
                <?php echo $bind,$data;?>/>
        </span>
	<?php if($has_units) { ?>
            <div class="selectwrapper noborder">
                <?php if(count($field['units'])>1):?>
                    <select name="<?php echo $field['id']; ?>_unit" <?php echo $select_id?> class="tb_lb_option tb_unit<?php if($isChild):?> tb_lb_option_child<?php endif;?>"<?php
                    echo $bind,$data;
                    ?>>
                        <?php
                ...
                <?php else:?>
                    ...
                    ?>
                    <span class="tb_unit" data-increment="<?php echo $field['units'][$unit]['increment'];?>" data-min="<?php echo $field['units'][$unit]['min'];?>" data-max="<?php echo $field['units'][$unit]['max'];?>"><?php echo strtolower($unit)?></span>
                <?php endif;?>
            </div>
            <?php
                ...
                if ($field['type'] === 'multi') {
                    ...
                    <div class="tb_input<?php echo!empty($field['pushed']) ? ' ' . $field['pushed'] : ''; ?>">
                        ...
                        <?php elseif ('radio' === $field['type']): ?>
                            ...
                            <div id="<?php echo $field['id']; ?>" class="tb_lb_option tb_radio_input_container<?php echo $option_js ? ' tb_option_radio_enable' : ''; ?>"<?php echo $control; ?>>
