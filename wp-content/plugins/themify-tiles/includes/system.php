...
class Themify_Tiles {
	...
	private function __construct() {
		...
		add_filter( 'themify_tiles_module_content', array( $this, 'format_text' ) );
	...
	public function builder_dependencies() {
		if( ! function_exists( 'themify_builder_module_settings_field' ) ) {
			include( THEMIFY_TILES_DIR . 'includes/themify-builder/includes/components/base.php' );
	...
	public function render_tiles( $group_id ) {
		...
		if( $post && $post->post_type == 'themify_tile' ) {
			...
			$output .= $this->load_view( $template, array(
				...
				'fluid_tiles_size' => get_post_meta( $group_id, 'tf_fluid_tiles_base_size', true ),				
				'post_id' => $group_id
			) );
		}
	...
	function themify_tiles_script_vars() {
		$options = get_option( 'themify_tiles' );
		$key = isset( $options['google_maps_key'] ) ? $options['google_maps_key'] : '';

		$l10n = apply_filters( 'themify_tiles_script_vars', array(
			'ajax_nonce'	=> wp_create_nonce('ajax_nonce'),
			'url'           => THEMIFY_TILES_URI,
		...
		$script = "var ThemifyTiles = " . wp_json_encode( $l10n ) . ';';
		return $script;
	}
	...
	public function admin_enqueue() {
		...
		wp_enqueue_style('themify-builder-combine-css', THEMIFY_TILES_URI . 'includes/themify-builder/css/combine.css', array() );
		wp_enqueue_style( 'themify-builder-toolbar-css', THEMIFY_TILES_URI . 'includes/themify-builder/css/toolbar.css', array() );
		wp_enqueue_script( 'themify-plupload', THEMIFY_URI . '/js/plupload.js', array('jquery'), false);
		wp_register_script( 'gallery-shortcode', THEMIFY_URI . '/js/gallery-shortcode.js', array( 'jquery' ), false, true );
		...
		wp_localize_script( 'themify-tiles-admin', 'ThemifyTilesAdminVars', array(
			'post_id' => $post->ID
		) );
	...
	function themify_do_metaboxes( $panels ) {
		$options = array(
			...
			array(
			"name" => "tf_fluid_tiles_base_size",
			"title" => __('Base Tile Size', 'themify-tiles'),
			...
			"type" => "dropdown",
			"meta" => array(
				array("value" => "", "name" => __('Default', 'themify-tiles'), "selected" => true),
				array("value" => '16', 'name' => __('16%', 'themify-tiles')),
				array("value" => '20', 'name' => __('20%', 'themify-tiles')),
				array("value" => '25', 'name' => __('25%', 'themify-tiles')),
				array("value" => '30', 'name' => __('30%', 'themify-tiles'))
			)
			),
			array(
	...
	function display_shortcode( $meta_box ) {
		...
		if( 'post.php' == $hook_suffix ) {
			...
			printf( '<br/><input type="text" value="%s" size="40" readonly="readonly" onclick="this.select()" />', esc_attr( '[themify_tiles group="' . $post->ID . '"]' ) );
			$post->ID !== $post->post_name && printf( '<br/><input type="text" value="%s" size="40" readonly="readonly" onclick="this.select()" />', esc_attr( '[themify_tiles group="' . $post->post_name . '"]' ) );
		} else {
