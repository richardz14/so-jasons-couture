	...
	ThemifyPageBuilder = {
		openGallery: function() {
			...
			$( 'body' ).on('click', '.tf-gallery-btn', function( event ){
				...
				var shortcode_val = $(this).closest('.tb_input').find('.tf-shortcode-input');
		...
		mediaUploader: function() {
			...
			// Field Uploader
			$( 'body' ).on('click', '.tb_media_uploader', function( event ){
				var $el = $(this);
				...
				file_frame.on( 'select', function() {
					...
					$el.closest('.tb_input').find('.tb_uploader_input').val(attachment.url).trigger( 'change' )
					.parent().find('.img-placeholder').empty()
					.html($('<img/>', {src: attachment.url, width: 50, height:50}))
					.parent().show();

					// Attached id to input
					$el.closest('.tb_input').find('.tb_uploader_input_attach_id').val(attachment.id);
				});
			...
			// delete button
			$( 'body' ).on('click', '.tb_delete_thumb', function(e){
				$(this).prev().empty().parent().hide();
				$(this).parents('.tb_input').find('.tb_uploader_input').val('');
				e.preventDefault();
			});
