            ...
            if( $posts->have_posts() ){
                while( $posts->have_posts() ){
                    ...
                    $layouts[] = array(
                            'title' => get_the_title(),
                            'slug' => $post->post_name,
                            'thumbnail' => has_post_thumbnail() ? get_the_post_thumbnail(null, 'thumbnail', array( 150, 150 ) ) : sprintf( '<img class="moje23" src="%s">',  get_template_directory_uri().'/themify/themify-builder/img/150x150.png' ),
                    );
                } 
            }
            wp_reset_postdata();
