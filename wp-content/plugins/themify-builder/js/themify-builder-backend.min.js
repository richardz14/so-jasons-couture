...
a(".editor-post-publish-panel__toggle").trigger("click"),b.redirectFrontend=c):a("#publish").trigger("click"):themifyBuilder.is_gutenberg_editor?(a(".editor-post-save-draft").trigger("click"),b.redirectFrontend=c):a("#save-post").trigger("click")};b._backendBuilderFocus=function(){a("#page-buildert").trigger("click");a("html, body").animate({scrollTop:a("#tb_toolbar").offset().top-a("#wpadminbar").height()},2E3)};a(function(){if(d.hasClass("post-php")&&a("#post-lock-dialog").length){if(!a("#post-lock-dialog").hasClass("hidden"))return;
Themify.LoadAsync(themifyBuilder.builder_url+"/js/themify-ticks.js",function(){d.hasClass("tb_restriction")?TB_Ticks.init(themifyBuilder.ticks,window).show():TB_Ticks.init(themifyBuilder.ticks,window).ticks()},null,null,function(){return"undefined"!==typeof TB_Ticks})}if("function"===typeof window.icl_copy_from_original){var b=window.icl_copy_from_original;window.icl_copy_from_original=function(a,c){b(a,c);jQuery.ajax({url:ajaxurl,type:"POST",data:{action:"themify_builder_icl_copy_from_original",
source_page_id:c,source_page_lang:a},success:function(a){"-1"!=a&&jQuery("#page-builder .themify_builder.themify_builder_admin").empty().append(a).contents().unwrap()}})}}});a(window).load(function(){function c(){var b=r+a("#page-builder").height(),c=a(window).scrollTop();return c>n&&c<b}function d(){f.width(f.width());n=h.offset().top;p=h.offset().left;r=Math.round(a("#page-builder").offset().top);k.height(f.outerHeight(!0));a(window).scroll(function(){l&&clearTimeout(l);l=setTimeout(e,15)}).resize(function(){n=
h.offset().top;p=h.offset().left;q="fixed"===m.css("position")?m.outerHeight(!0):0;f.width(a("#page-builder .themify_builder_admin").width()).css("top",c()?q:"");k.height(f.outerHeight(!0))})}function e(){l=null;c()?(f.addClass("tb_toolbar_fixed").css({top:q,left:p}),k.css("display","block")):(f.removeClass("tb_toolbar_fixed").css({top:0,left:0}),k.css("display","none"))}b.render();var f=a("#tb_toolbar"),k=a("#tb_module_tmp"),h=a("#tb_scroll_anchor"),n=0,p=0,l=null,r=0,m=a("#wpadminbar"),q="fixed"===
m.css("position")?m.outerHeight(!0):0;if(0<f.length)if(f.is(":visible")&&!themifyBuilder.is_gutenberg_editor)d();else a(".themify-meta-box-tabs a").one("click",function(){"page-buildert"===a(this).attr("id")&&d()});sessionStorage.getItem("focusBackendEditor")&&(b._backendBuilderFocus(),sessionStorage.removeItem("focusBackendEditor"))})}})(jQuery);
